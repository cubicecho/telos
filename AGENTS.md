...
│   ├── global.css           # Imports the tokens; page colours; local patches (cubeui#102, #103)
│   ├── metro.config.js
│   └── postcss.config.mjs
...
imports it and sets the page background and border colour. It also carries two
local patches: a form-field font reset (cubicecho/cubeui#103), without which a
raw `<input>` draws in Arial and a `<textarea>` in monospace, and the web
calendar's colours mapped to the tokens (cubicecho/cubeui#102), without which
react-day-picker draws in its stock blue.
